...
- Для управления удобностью работы с клавиатуры использовал `tabindex`
- Для a11y исползовал тэги `aria-label` - для пояснения смысла блоков, для скринридеров. `aria-hidden` - для скрытия ненужного контетна от скринридеров.
